...
class IETModel(SRModel):
    def test(self):
        ...
        if h * w < patch_size ** 2:
            ...
            self._test_split(patch_size=patch_size)
    ...
    def _test_split(self, patch_size=256):
        _, C, h, w = self.lq.size()
        split_token_h = h // patch_size + 1  # number of horizontal cut sections
        split_token_w = w // patch_size + 1  # number of vertical cut sections
        # padding
        ...
        if hasattr(self, 'net_g_ema'):
            ...
            with torch.no_grad():
                ...
                for chop in img_chops:
                    ...
                    outputs.append(out)
                _img = torch.zeros(1, C, H * scale, W * scale, device=outputs[0].device, dtype=outputs[0].dtype)
                # merge
                for i in range(ral):
                    for j in range(row):
