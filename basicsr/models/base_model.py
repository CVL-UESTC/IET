...
class BaseModel():
    ...
    def get_optimizer(self, optim_type, params, lr, **kwargs):
        if optim_type == 'Adam':
            ...
            betas = kwargs.get('betas', None) or (0.9, 0.99)
            weight_decay = kwargs.get('weight_decay', None) or 0.001
            scale = kwargs.get('scale', None) or 10

            hidden_weights = [p for p in params if p.ndim > 3]
            hidden_gains_biases = [p for p in params if p.ndim < 3]
            param_groups = [
                dict(params=hidden_weights, use_muon=True,
                    lr=scale*lr, weight_decay=weight_decay),
